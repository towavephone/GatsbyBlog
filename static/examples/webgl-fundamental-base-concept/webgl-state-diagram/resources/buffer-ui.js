...
import { collapseOrExpand, createExpander, expand, flashSelfAndExpanderIfClosed, makeDraggable } from './ui.js';
...
export function createBufferDisplay(parent, name /*, webglObject */) {
  ...
  const updateSubData = (data, offset) => {
    ...
    expand(dataExpander);
    flashSelfAndExpanderIfClosed(dataElem);
    const oldData = dataElem.textContent
      .replace(', ...', '')
      .split(', ')
      .map(parseFloat);
    ...
    for (const attachmentPoint of attachmentPoints) {
      ...
      switch (type) {
        ...
        case gl.TEXTURE:
          ...
          rawFace = gl.getFramebufferAttachmentParameter(
            target,
            attachmentPoint,
