...
import { formatG, formatX2, formatBoolean, formatUniformValue, helpToMarkdown } from './utils.js';
import {
  formatWebGLObject,
...
export function getStateTables(isWebGL2) {
  ...
  const stencilOps = helpToMarkdown(
    `--KEEP--, --ZERO--, --REPLACE--, --INCR--, --INCR_WRAP--, --DECR--, --DECR_WRAP--, --INVERT--`
  );
  const stencilFuncSnippet = (face) =>
    helpToMarkdown(`
  ---js
