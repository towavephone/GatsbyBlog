...
import { addElem, createTable, formatUniformValue, getColorForWebGLObject, helpToMarkdown } from './utils.js';

import { formatWebGLObject, getWebGLObjectInfo } from './context-wrapper.js';

import { createExpander, expand, makeDraggable, updateElemAndFlashExpanderIfClosed } from './ui.js';

import { createStateGrid, createStateTable, updateStateTable } from './state-table.js';
...
export function createGlobalUI(globalStateElem) {
  ...
  const globalUI = {
    ...
    ...(globals.isWebGL2 && {
      transformFeedbackState: createStateUI(
        globalState.transformFeedbackState,
        globalStateElem,
        'transform feedback',
        globalStateQuery
      ),
      uniformBufferBindingsState: createUniformBufferBindings(
        globalStateElem,
        Math.min(8, gl.getParameter(gl.MAX_UNIFORM_BUFFER_BINDINGS))
      )
    }),
    ...
    ...(globals.isWebGL2 && {
      framebufferState: createStateUI(
        globals.stateTables.framebufferState,
        globalStateElem,
        'read/draw buffers',
        globalStateQuery
      )
