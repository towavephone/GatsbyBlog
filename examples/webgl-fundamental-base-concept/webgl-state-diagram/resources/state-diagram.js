...
import { addElem, flash, removeFlashes } from './utils.js';
...
import { isBadWebGL2, init as initWebGL } from './webgl.js';
import { globals } from './globals.js';
import { createShaderDisplay, createProgramDisplay } from './program-ui.js';
import { createSamplerDisplay, createTextureDisplay } from './texture-ui.js';
import { createBufferDisplay, createFramebufferDisplay, createRenderbufferDisplay } from './buffer-ui.js';
import { createVertexArrayDisplay } from './vertex-array-ui.js';
import { createTransformFeedbackDisplay } from './transform-feedback-ui.js';
...
export default function main({ webglVersion, examples }) {
  ...
  gl = document
    ...
    .getContext(webglVersion, { preserveDrawingBuffer: true }); /* eslint-disable-line */
  ...
  function getQueryForBufferBindPoint(gl, bindPoint) {
    switch (bindPoint) {
      ...
      default:
        throw new Error(`unsupported bind point: ${glEnumToString(gl, bindPoint)}`);
    }
  }
  ...
  function updateFramebufferAttachments(target) {
    const framebuffer = gl.getParameter(
      target === gl.FRAMEBUFFER ? gl.FRAMEBUFFER_BINDING : gl.READ_FRAMEBUFFER_BINDING
    );
    const { ui } = getWebGLObjectInfo(framebuffer);
