...
import { createStateTable, updateStateTable } from './state-table.js';
  ...
  const scan = () => {
    ...
    for (let ii = 0; ii < numAttribs; ++ii) {
      ...
      if (isCurrent) {
        ...
        arrows.push(
          arrowManager.add(tr, target, getColorForWebGLObject(vao, target, index / 8), {
            startDir: 'right',
            endDir: 'right',
  ...
  const scan = () => {
    ...
    for (let ii = 0; ii < numAttribs; ++ii) {
      ...
      if (isCurrent && tfInfo) {
        ...
        arrows.push(
          arrowManager.add(tr, target, getColorForWebGLObject(tf, target, ii / 8), {
            startDir: 'right',
            endDir: 'left',
            attrs: { 'stroke-dasharray': '2 4' }
          })
    ...
    locationInfos.forEach((locationInfo, ndx) => {
      ...
      if (bindPoint) {
        ...
        if (isCurrent) {
          ...
          const target = globals.globalUI.textureUnits.elem.querySelector('tbody').rows[
            value
          ]; /*.cells[bindPointIndex]; */
    ...
    if (globals.isWebGL2) {
      ...
      for (const blockSpec of Object.values(blockSpecs)) {
        ...
        if (isCurrent) {
          ...
          blockSpec.arrow = arrowManager.add(blockNameCell, target, getColorForWebGLObject(null, target), {
            startDir: 'left',
            endDir: 'right',
            attrs: { 'stroke-dasharray': '2 4' }
          });
